...
namespace MoveThing
{
    ...
    public class Dungeon
    {
        ...
        public bool MakeRoom(int x, int y, int xlength, int ylength, Direction direction)
        {
            ...
            int xlen = this.GetRand(3, xlength);
            int ylen = this.GetRand(3, ylength);
        ...
        public void SaveDungeon(string path, string name)
        {
        ...
        public bool CreateDungeon(int inx, int iny, int inobj)
        {
            ...
            *******************************************************************************/
            ...
            this.MakeRoom(this._xsize / 2, this._ysize / 2, maxRoomLength, maxRoomHeight, RandomDirection()); // getrand saken f????r att slumpa fram riktning p?? rummet
            ...
            for (int countingTries = 0; countingTries < 1000; countingTries++)
            {
                ...
                if (validTile.HasValue)
                {
                    ...
                    { // a new room
                        if (this.MakeRoom(newx + xmod, newy + ymod, maxRoomLength, maxRoomHeight, validTile.Value))
                        {
                    ...
                    { // new corridor
                        if (this.MakeCorridor(newx + xmod, newy + ymod, maxCorridorLength, validTile.Value))
                        {
